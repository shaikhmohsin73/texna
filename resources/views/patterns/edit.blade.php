...
@section('content')
    ...
    <div class="form-container">
        ...
        <h2 class="mb-4 text-center fw-bold">Edit Pattern</h2>
        ...
        <form action="/patterns/{{ $pattern->id }}/update" method="POST" enctype="multipart/form-data" novalidate>
            @csrf

            <div class="form-floating mb-4">
                <input type="text" name="pattern_no" id="pattern_no" class="form-control" placeholder="Pattern No"
                    required value="{{ old('pattern_no', $pattern->pattern_no) }}">
                <label for="pattern_no">Pattern No</label>
            </div>

            <div class="mb-4">
                <label for="pattern_file" class="form-label fw-semibold">Replace PDF (optional)</label>
                <input type="file" name="pattern_file" id="pattern_file" class="form-control" accept="application/pdf">
                <div class="form-text">Upload new PDF only if you want to replace the old one. Max 2MB.</div>
            </div>
